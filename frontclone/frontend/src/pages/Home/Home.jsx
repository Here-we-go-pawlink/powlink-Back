        ...
        <div className="emotion-summary card">
            ...
            <div className="emotion-chips">
              {stats?.emotionDistribution?.length > 0
                ? stats.emotionDistribution.slice(0, 4).map((e) => (
                    <span key={e.emotion} className="chip">
                      {e.emotion} {e.percentage}%
                    </span>
                  ))
                : <span className="chip">일기를 작성하면 감정 분석이 시작돼요</span>
              }
            </div>
          </div>
          ...
          <div className="summary-item">
            ...
            <div className="kw-chips">
              {stats?.keywords?.length > 0
                ? stats.keywords.slice(0, 5).map((k) => (
                    <span key={k.text} className="kw-chip">#{k.text}</span>
                  ))
                : <span className="kw-chip">#아직 없음</span>
              }
            </div>
          </div>
          <div className="summary-divider" />
          <div className="summary-item summary-item-wide">
            ...
            <span className="summary-analysis">
              {stats?.aiInsights?.summary ?? 'AI 분석이 완료되면 이번 달 감정 인사이트를 보여드릴게요.'}
            </span>
          </div>
        </div>
